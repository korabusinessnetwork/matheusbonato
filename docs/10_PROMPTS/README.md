...
estática e determinística.

Este arquivo existe para deixar a ausência explícita, e não parecer documentação
esquecida.

## Onde a IA aparece (e onde não aparece)

- **Na construção**: este projeto foi construído com apoio de agente, seguindo a
  skill `fundacao-de-projeto`. Isso é processo de desenvolvimento, não feature.
- **Em produção**: nada. Nenhuma chamada a modelo, nenhuma chave de API.
